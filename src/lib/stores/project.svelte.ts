  ...
  execute(): void {
    ...
    this.store.setTrack({
      ...
      keyframes: track.keyframes.map((kf) =>
        kf.id === this.newKeyframe.id ? this.newKeyframe : kf
      ),
    });
  }
  undo(): void {
    ...
    this.store.setTrack({
      ...track,
      keyframes: track.keyframes.map((kf) =>
        kf.id === this.oldKeyframe.id ? this.oldKeyframe : kf
      ),
    });
  }
  serialize(): SerializedCommandData {
...
class ReorderChannelsCommand implements Command {
  ...
  serialize(): SerializedCommandData {
    return {
      ...
      description: this.description,
      payload: {
        oldOrder: this.oldOrder.map((c) => c.id),
        newOrder: this.newOrder.map((c) => c.id),
      },
    };
  }
}
...
class ProjectStoreClass {
  ...
  moveKeyframe(id: string, newTimestampMs: number): void {
    ...
    if (!kf) return;
    undoStore.execute(
      new MoveKeyframeCommand(this, id, kf.timestampMs, Math.max(0, newTimestampMs))
    );
  }
  ...
  addSceneMarker(
    timestampMs: number,
    label: string,
    type: SceneMarker['type'] = 'MARKER_MANUAL'
  ): SceneMarker {
